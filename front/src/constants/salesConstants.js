export const SALES_REQUEST = "SALES_REQUEST";
export const SALES_SUCCESS = "SALES_SUCCESS";
export const SALES_FAIL = "SALES_FAIL";

export const ADD_SALES_REQUEST = "ADD_SALES_REQUEST";
export const ADD_SALES_SUCCESS = "ADD_SALES_SUCCESS";
export const ADD_SALES_FAIL = "ADD_SALES_FAIL";

export const REMOVE_SALE_REQUEST = "REMOVE_SALE_REQUEST";
export const REMOVE_SALE_SUCCESS = "REMOVE_SALE_SUCCESS";
export const REMOVE_SALE_FAIL = "REMOVE_SALE_FAIL";

export const CONFIRM_SALES_REQUEST = "CONFIRM_SALES_REQUEST";
export const CONFIRM_SALES_SUCCESS = "CONFIRM_SALES_SUCCESS";
export const CONFIRM_SALES_FAIL = "CONFIRM_SALES_FAIL";

export const RESET_SALES = "RESET_SALES";
