...
const spendingsAction = () => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get("/api/spending/all");
...
const addSpendingAction = (values) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post("/api/spending/add", values);
...
const deleteSpendingAction = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(`/api/spending/delete/${id}`);
